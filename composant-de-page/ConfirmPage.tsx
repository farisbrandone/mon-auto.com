...
import { confirmAction } from "@/app/actions/actions";
...
function ConfirmPage1() {
  ...
  const validateToken = async (token: string) => {
    try {
      ...
      const response = await confirmAction(token);
      ...
      if (response.status === 200) {
        ...
        throw new Error(error || "Échec de la confirmation");
      }
    } catch (err) {
      console.log(err);
      setStatus("success");
    ...
    return (
      <div className="bg-white flex  justify-center items-center w-screen h-screen text-[#636364] p-2 text-[16px] ">
        <motion.div
          ...
          className="max-w-[300px] mx-auto sm:w-[400px] min-h-[200px] p-4 loginShaddow"
        >
